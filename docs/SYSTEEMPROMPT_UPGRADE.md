...
**UI-klacht: opgelost (2026-08-19, meerdere iteraties op basis van live testen).** Wijzigingen in `SparClient.tsx`:
1. De sticky invoerbalk verdwijnt volledig tijdens het genereren van een antwoord, en komt pas terug zodra de gebruiker richting het einde van dat antwoord scrolt (`readyForInput`-state + `IntersectionObserver` op de bestaande `bottomRef`).
2. Automatisch meescrollen tijdens het streamen van een antwoord (zoals elke andere chat-app), tenzij de gebruiker zelf scrolt. Eerste versie detecteerde dat via het generieke `scroll`-event, wat fout ging: de eigen programmatische smooth-scroll van de app veroorzaakte tijdens zijn animatie ook `scroll`-events, die dan verkeerd als "gebruiker scrolt weg" werden gelezen, waardoor meescrollen soms helemaal niet werkte. Vervangen door `wheel`/`touchmove`-detectie, die alleen van echte gebruikersinput komt.
3. De balk overlapte bij het verschijnen de laatste regels van het antwoord. Eerste poging: een `useLayoutEffect` die het beeld bij het verschijnen exact de hoogte van de balk terugschoof. Bleek in de praktijk onbetrouwbaar (nog steeds overlap zichtbaar bij live testen). Structureel vervangen: `paddingBottom` op `.spar-page` reserveert weer permanent ruimte voor de balk zolang een gesprek loopt (ongeacht of de balk zelf nu zichtbaar is), en `bottomRef` kreeg een `scroll-margin-bottom` van dezelfde grootte, zodat het meescrollen (punt 2) daar vanzelf al rekening mee houdt. Geen losse correctiestap meer die kan mistimen.
