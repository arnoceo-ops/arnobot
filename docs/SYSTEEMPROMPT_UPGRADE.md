...
- [x] **Gestructureerde bevindingen met trendtabel** (2 september). Het expertpanel sluit zijn antwoord af met een machineleesbaar `<<BEVINDINGEN>>`-blok: `slug | label | ernst | trend | toelichting` per structureel punt, max 6. Bewust in de bestaande `expertpanel_text`-kolom, geen migratie: het blok wordt uit de weergave gestript (UI en e-mail) en de vorige slug plus label worden teruggevoerd zodat ze stabiel blijven over maanden. `lib/metaAnalyseTrend.ts` bevat `parseBevindingen` (tolerant voor het oude 4-velds formaat), `stripBevindingenBlok`, `BEVINDINGEN_INSTRUCTIE`. Admin-scherm: `TrendTabel` boven het archief, kaart per bevinding met leesbaar label, ernsttag (HOOG/MIDDEN/LAAG) en toelichting, plus een bewegingskolom per analyse zodra er meer dan één is (○ nieuw, ↑ verbeterd, = gelijk, ↓ verslechterd). De analyse van 1 september heeft een baseline-blok gekregen. Smoke-test tegen Fable 5 en parse-test geslaagd. Visuele check in de browser staat nog open (lokale dev-server faalt op een pre-existing Tailwind/Turbopack CSS-fout in `app/globals.css`, raakt elke pagina, niet deze code).
...
- [x] Kennisbankdoc `verifieer-eerst-ruimte-niet-obstakel.md`: laatste alinea gesoftend zodat hij overeenkomt met de prompt (suggestie of vraag over de ruimte, geen harde confrontatie). 4 oude chunks verwijderd, 4 nieuwe ge-embed via `scripts/embed-single-doc.mjs` (source: "Kennisbank: Verifieer eerst, dan pas adviseer"). Geverifieerd: oude harde zin weg, nieuwe zin aanwezig.
