...
- [x] **Gestructureerde bevindingen met trendtabel** (2 september). Het expertpanel sluit zijn antwoord af met een machineleesbaar `<<BEVINDINGEN>>`-blok (slug, ernst, trend, toelichting per structureel punt, max 6). Bewust in de bestaande `expertpanel_text`-kolom, geen migratie: het blok wordt uit de weergave gestript (UI en e-mail) en de vorige slugs worden teruggevoerd zodat de labels stabiel blijven. `lib/metaAnalyseTrend.ts` bevat `parseBevindingen`/`stripBevindingenBlok`/`BEVINDINGEN_INSTRUCTIE`. Admin-scherm toont een `TrendTabel` boven het archief (rij per slug, kolom per analyse, kleur = ernst, teken = beweging). De analyse van 1 september heeft een baseline-blok gekregen zodat de eerstvolgende echte run (rond 1 oktober) ertegen kan trenden. Smoke-test tegen Fable 5 geslaagd. Slugs met underscores zodat de streepjesregel niet in het geding komt.
