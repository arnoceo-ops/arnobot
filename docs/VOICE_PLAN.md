...
**Voice-out tijdelijk uitgezet voor iedereen behalve Arno zelf (besloten en gebouwd 2026-08-13).** Aanleiding: Arno logde in via zijn eigen LinkedIn-account (`linkedin@royaldutchsales.com`) en zag daar voice-out functioneren, wat hem deed vragen of dat ook bij andere gebruikers zo was. Bleek zo te zijn, en bewust zo gebouwd (`proxy.ts` zet `plan: 'premium'` voor elke nieuwe trial-aanmelding, en `hasVoiceAccess()` gaf daardoor iedereen ongelimiteerde paid-toegang, ook trial-gebruikers, zie het "Openstaand actiepunt" hierboven over de nooit bereikte `TRIAL_VOICE_CHAR_CAP`). Arno wil voice-out nu eerst uitsluitend zelf verder testen, tot de commerciële livegang (nog niet gepland, zie geheugen `project-launch-date-postponed`, vermoedelijk sept/okt 2026).
**Bewust besluit, expliciet nagevraagd:** dit geldt ook voor de 6 bestaande, echt betalende gebruikers die voice al gebruikten (`thijs@tenshare.nl`, `rtesfahuny@hotmail.com`, `stefanie_coster@hotmail.com`, `dyannethornleeson@gmail.com`, `charles.deklerk@xs4all.nl`, `erik.van.den.berg@xs4all.nl`, gevonden via `approved_users` met `paid_at IS NOT NULL`), niet alleen voor nieuwe/trial-aanmeldingen. Ter vergelijking eerst voorgelegd of dit de bedoeling was (zij verliezen een functie waar ze al voor betalen, zonder aankondiging), Arno bevestigde expliciet: ja, iedereen behalve zijn eigen twee accounts.
**Implementatie:** `lib/voice.ts` (`VOICE_LAUNCH_RESTRICTED`, `VOICE_ALLOWED_USER_IDS`, `isVoiceLaunchAllowed()`) is de enige plek met de allowlist, met een duidelijke verwijderinstructie in de code zelf voor zodra Voice commercieel live gaat. Toegepast op twee niveaus: `hasVoiceAccess()` (server-side enforcement, dekt zowel `/api/chat-voice` als `/api/tts-voice`, want beide roepen die functie al aan) én `app/bot/page.tsx` (`voiceEnabled`, verbergt de voice-toggle in de UI voor iedereen buiten de allowlist, i.p.v. een zichtbare knop die vervolgens een foutmelding geeft). De admin-only testroute (`/bot/admin/voice-test`) is bewust ongemoeid gelaten, die gebruikt `hasVoiceAccess()` niet en blijft dus altijd werken voor Arno, ongeacht deze restrictie.
**Allowlist:** `MANUAL_TEST_USER_ID` (test@arno.bot, uit `lib/internalTestAccounts.ts`) en het userId van `linkedin@royaldutchsales.com` (Arno's echte LinkedIn-account, elders in de codebase bekend als `BOUWER_EMAIL`, hier hardcoded als userId opgezocht via Supabase omdat de betrokken routes geen e-mailadres ophalen).
**Niet aangepast:** marketingcopy op `/prijzen`/`docs/ABONNEMENTEN.md` die voice als Pro/Team-feature beschrijft, blijft ongewijzigd staan (correct voor na de livegang, alleen de daadwerkelijke toegang is nu tijdelijk beperkt). Geen nieuwe kolom of feature-flag in Supabase, puur een hardcoded allowlist in code, bewust tijdelijk en eenvoudig terug te draaien.
**Eerstvolgende stap:** geen. Bij de daadwerkelijke commerciële livegang: `VOICE_LAUNCH_RESTRICTED` op `false` zetten (of het hele blok verwijderen) in `lib/voice.ts`.

*Abonnementsherstructurering:* `tier`(`'basis'|'pro'`)+`voice_enabled` volledig vervangen door één kolom `plan` (`'basis'|'premium'|'team'`) op `approved_users`. Alle codeplekken omgezet, admin-toggle vervangen (`PlanToggle.tsx`, 3-way, `/api/admin/plan`). **Migratie 2 uitgevoerd en bevestigd:** kolommen `tier` en `voice_enabled` zijn gedropt, de omzetting is daarmee volledig afgerond, geen legacy-kolommen meer.

**Kritieke bug gevonden en gefixt, zelfde dag:** de nieuwe `plan`-kolom kreeg aanvankelijk `DEFAULT 'basis'`, en `proxy.ts` zette `plan` nooit expliciet bij provisioning. Nieuwe trial-aanmeldingen kregen daardoor stilzwijgend een beperkte ervaring (geen coaching, geen voice, 25 berichten/dag) in plaats van de bedoelde volledige functionaliteit tijdens de 30-dagen-trial (zoals vóór de migratie ook al zo werkte, toen stond `tier` default op `'pro'`). Ontdekt en gefixt vóórdat nieuwe gebruikers hierdoor geraakt zijn: `plan: 'premium'` nu expliciet in `proxy.ts`, kolom-default in Supabase gelijkgetrokken naar `'premium'`. Zie `docs/ABONNEMENTEN.md` voor de volledige toelichting.

*Gesprek-met-Arno-boeking:* Calendly (event `arnobot/ab`) als scheduling-tool. Nieuwe kolom `arno_call_booked_at` op `approved_users`. `app/bot/gesprek/page.tsx`: eerst een Calendly-embed geprobeerd, teruggedraaid naar een externe link (nieuw tabblad) omdat kleuraanpassing aan het donkere thema een betaald Calendly-plan vereist en de embed een storende interne scroll gaf. Robuuste matching via `utm_content=<Clerk-userId>` in de link (komt terug in de webhook-payload `payload.tracking.utm_content`, onafhankelijk van wat iemand zelf in het Calendly-formulier invult), met e-mailmatching als fallback voor boekingen buiten de app om. Webhook `app/api/webhooks/calendly/route.ts` (HMAC-SHA256-signatuurverificatie + 5 min replay-venster). Reminder-e-mail `dag21_gesprek` toegevoegd aan `lib/email-templates.ts` + trigger in de bestaande `app/api/cron/trial-emails/route.ts`-loop.

*Upgrade-flows (nieuw t.o.v. het oorspronkelijke plan, ontstaan tijdens de sessie):* `/bot/upgrade` (basis→premium en premium→team, mailto + klik-tracking, geen self-serve betaling). `/upgrade` (ArnoLive/ArnoPrime, Arno's eigen consulting-aanbod, los van ArnoBot-abonnementen) verplaatst naar `/arnolive` om de naamsclash op te lossen. Boekingskaart op de accountpagina weer verwijderd op Arno's verzoek: het gesprek loopt alleen via de dag21-mail en de coaching-pagina, niet via een permanente accountpagina-kaart.

Build (`npm run build`), typecheck en lint zijn na elke stap gecontroleerd. Regressietest is door Arno zelf op productie gedaan, inclusief screenshots per stap; alle gevonden problemen (verouderde "Pro"-copy, teller die weg moest, knopbreedtes, het embed-scrollprobleem, de trial-plan-bug) zijn binnen deze sessie gevonden en gefixt.

**Eerstvolgende stap:** geen actief bouwwerk. Open, niet-blokkerend punt: een echte proefboeking om de webhook-matching end-to-end te bevestigen is bewust overgeslagen (Arno vertrouwt de matching-logica), kan alsnog als losse verificatie.
