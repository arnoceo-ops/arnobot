...
**Waar we staan:** Fase 1 staat al live. **Punt 7 (PDF-export teamniveau) afgerond.** **Punt 6 gebouwd**: tabel `arnobot_salesbaas_coaching` + gedeelde `isConfirmedTeambaas(userId)`-helper (`lib/teamAccess.ts`), zie de sectie "Raamwerk: rollen × disciplines" hieronder voor de volledige herziening. RLS-incident onderweg gevonden en volledig gefixt (zie CLAUDE.md sectie 1). **Punt 1 gebouwd en door Arno getest/bijgestuurd (2026-08-20):** `arnobot_1on1_log` uitgebreid met `actie`/`actie_status` ('ja'/'nee'/'skip', zelfde vocabulaire als `arnobot_blog_sessions.actie_status`). **Besluit: één vrij tekstveld per 1:1, geen losse gestructureerde acties** (optie A, expliciet gekozen boven een array/meerdere-acties-per-1:1-redesign, blijft consistent met hoe de rest van de app al werkt: één AANDACHTSPUNT, niet meerdere). Het "openstaande actie"-blok (GEDAAN/NIET GEDAAN/OVERSLAAN) staat, na twee correcties op basis van Arno's feedback, vast onderaan "1:1 GESCHIEDENIS" op `app/bot/team/lid/[userId]/page.tsx`, zichtbaar bij het openen van de pagina zelf, niet gekoppeld aan het genereren van een nieuwe agenda (dat voorkwam ook een dubbele-databron-bug). Het "OPENSTAAND"-statuslabel in de geschiedenislijst is verwijderd (zette op het verkeerde been zonder context). PDF-export (`OneOnOnePdfDocument.tsx`) toont nu ook de actie + status, lokaal gerenderd en visueel geïnspecteerd. `TeamClient.tsx` toont een nieuw, bewust feitelijk (geen AI-tekst) blok "JOUW 1:1-RITME": 1:1's laatste 30 dagen, follow-through-percentage, aantal openstaande acties ouder dan 2 weken. **Auto-save toegevoegd (2026-08-20):** een gegenereerde agenda ging voorheen spoorloos verloren als de manager niet expliciet op "BEWAAR DEZE 1:1" klikte, botste met het bestaande auto-save-i.p.v.-bevestiging-principe en met de dossier-waarde van consistente 1:1's (zie de Fase 2B-sectie hieronder). De BEWAAR-knop is verwijderd: de agenda wordt nu direct na genereren opgeslagen, en de actie auto-saved gedebouncet (800ms) tijdens het typen, zonder bevestiging. Aangevuld met een `sendBeacon`/`pagehide`-vangnet voor de smalle rand-case van razendsnel typen + direct wegnavigeren binnen de debounce-tijd. Ook de actie- en notitietekstvelden zelf herstijld: groeien nu automatisch mee met de inhoud (zelfde patroon als `SparClient.tsx`'s hoofdgespreksveld: `resize:none; overflow:hidden; field-sizing:content` + JS-hoogteberekening), niet meer een interne scrollbar.
**Eerstvolgende stap:** punt 5 (coachende rol richting Thijs zelf als coach), bouwt op 1 + 6. Concreet plan besproken, nog niet gebouwd, Arno heeft meer vragen (2026-08-20).
...
**Nieuw gevonden, apart, bewust NOG NIET gebouwd (optie 2 gekozen boven het nu meenemen):** het bestaande Mindset/Systeem/Actie-profiel wordt gevoed door ál iemands ArnoBot-gesprekken, ongeacht waar die over gaan. Voor een pure teambaas die nooit over verkopen praat maar wel met ArnoBot praat over leiderschapsvraagstukken, probeert het systeem hem dus nog steeds te scoren op een verkooplens die niet op hem van toepassing is. Open vraag die dit ook lastig maakt: wat gebeurt er met iemand die wél verkoopt én als teambaas meekijkt (zoals Thijs zelf), heeft die dan bewust twee aparte profielen? Nog niet beantwoord, apart traject, niet in punt 5 meenemen.
