...
**Gebouwd (2026-08-20):** tabel `arnobot_salesbaas_coaching` (kolommen: `strategy_score`/`strategy_diagnose`, `people_score`/`people_diagnose`, `execution_score`/`execution_diagnose`, `voortgang`, `used_1on1_ids`), en `lib/teamAccess.ts` met `isConfirmedTeambaas(userId)`. **Openstaand actiepunt:** RLS op deze nieuwe tabel nog niet expliciet ingesteld (de rest van de gebruikerstabellen kreeg dit in juli 2026, zie CLAUDE.md sectie 1), bij de volgende maandcheck of vóór punt 5 alsnog toevoegen, zelfde patroon als `arnobot_coaching`.
