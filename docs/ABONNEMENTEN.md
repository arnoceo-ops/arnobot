...
**Tot het template af is:** `/command` werkt onverminderd voor zijn kernfunctie, de aanvraag wordt opgeslagen in Supabase en Arno krijgt een interne meldmail met alle gegevens. Faalt de DocuSeal-aanroep (wat nu per definitie gebeurt zolang het template niet klopt), dan blijft de aanvraag gewoon bewaard en ziet Arno dat expliciet terug in die meldmail, dus niets gaat stil verloren. Offerte dan gewoon zelf handmatig versturen, zoals voorheen.
