...
**Eerstvolgende stap:** het DocuSeal-template (2026-07-24) handmatig via Word/Google Docs opbouwen liep vast op steeds terugkerende, onzichtbare opmaakproblemen (stray `{{`, velden die overlappen, velden die aan elkaar plakken), waarschijnlijk veroorzaakt door hoe Word/Google Docs tekst intern in losse "runs" opknipt bij typen/plakken, niet door de tagged-DOCX-aanpak zelf. Bewust gepauzeerd, geen tijd meer aan besteden op deze manier. Bij hervatten: het .docx-bestand programmatisch genereren vanuit code (bijv. met een library als `docx` in Node) i.p.v. handmatig typen in Word/Google Docs, zodat de tags gegarandeerd correct en geïsoleerd in het bestand staan. De API-koppeling zelf (`lib/docusealOffer.ts`, `app/api/command-aanvraag/route.ts`) staat al klaar en getest, template-ID 5160507, die hoeft niet opnieuw gebouwd te worden, alleen het brondocument. Daarna: DocuSeal-webhook voor automatische Supabase-inrichting bij ondertekening, seat-wijzigingen na ondertekening (zie geheugen `project-team-pricing`), Basis-retentieflow, Circle, betaalprovider (bewust in die volgorde, laatste twee bewust helemaal achteraan).

**Tot het template af is:** `/command` werkt onverminderd voor zijn kernfunctie, de aanvraag wordt opgeslagen in Supabase en Arno krijgt een interne meldmail met alle gegevens. De automatische DocuSeal-verzending zelf staat sinds 2026-07-24 bewust uit via `OFFERTE_AUTOMATISERING_ACTIEF = false` in `lib/docusealOffer.ts`, ontdekt na een live test die liet zien dat een echte aanvrager een offerte met kapotte opmaak (overlappende velden) zou ontvangen. Zet die schakelaar pas op `true` nadat het brondocument geverifieerd klopt (test opnieuw met "ZELF ONDERTEKENEN" in DocuSeal). Offerte tot die tijd gewoon zelf handmatig versturen, zoals voorheen.
