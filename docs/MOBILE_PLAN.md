...
**Eerstvolgende stap:** Google Play-account aanmaken (en meteen verifiëren hoe de verplichte closed-testing-periode voor het gekozen accounttype werkt, zie Openstaande vragen, dat heeft zelf ook doorlooptijd). Apple Developer-account (Individual, geen D-U-N-S nodig) en Mac-regeling kunnen later, wanneer de iOS-kant aan de beurt is. Zodra het Google Play-account er is: Capacitor toevoegen aan `package.json` en de eerste "remote laden"-shell opzetten voor Android (zie Architectuurbesluit hieronder). Stemopnames en pricingpagina volgen pas ná de mobiele app.
