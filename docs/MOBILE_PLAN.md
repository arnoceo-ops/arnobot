...
- **App-icoon:** de bestaande `public/arnobot-logo.png` is een brede tekst-wordmark (ARNO+BOT naast elkaar), niet geschikt als vierkant app-icoon. De bestaande `app/favicon.ico` is een simpel zwart driehoekje zonder duidelijke merkherkenning, ook niet geschikt. Er moet een nieuw, vierkant icoon gemaakt worden, consistent met de bestaande merkstijl (Bebas Neue, amber #f59e0b op donker #111827). Voor nu: een eenvoudig placeholder-monogram volstaat om te kunnen testen, een gepolijst definitief icoon is een latere stap vóór de daadwerkelijke store-submissie.

**Eerstvolgende stap:** placeholder-app-icoon maken en toepassen via Android Studio's Image Asset-tool. Daarna verdere test van de basisflow (chat, sparren, navigatie) en de mic-knop zodra er een werkende USB-kabel is.
