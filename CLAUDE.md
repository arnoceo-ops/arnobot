...
- **Openstaand (gevonden bij de eerste run, 2026-08-12):** `app/api/tts/route.ts` (OpenAI TTS, `tts-1-hd`) heeft nergens in de codebase een aanroeper, ook niet in `android/`. Eén commit sinds 10 juni 2026, vermoedelijk vervangen door de latere ElevenLabs `tts-voice`-feature en nooit opgeruimd. Staat nog wel in de privacypagina en het beveiligingsdocument als actieve verwerking. Niet zelf verwijderd: eerst bevestigen of dit klopt, dan in dezelfde actie de documentatie meenemen.
