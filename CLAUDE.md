...
**Verbreed (2026-08-29):** deze schrijfregel geldt niet alleen bij een compleet nieuwe capability, maar ook bij een **merkbare gedrags- of toonwijziging aan een bestaande gebruiker-gerichte feature of AI-output** (bijvoorbeeld: de toon van een AI-gegenereerde tekst wordt herzien, een drempel of zichtbaarheidsregel verandert, een bestaande knop of flow gedraagt zich anders). In dezelfde commit: werk het relevante deel van `docs/ARNOBOT_OVERZICHT.md` bij (of voeg de feature toe als die er nog niet in stond), en als de wijziging een route/feature raakt die in `docs/TECHNICAL_HANDOVER.md` beschreven staat (routetabel, featurebeschrijving, modelinventaris-rij), werk die beschrijving daar ook meteen bij. Aanleiding: een toon- en drempelwijziging aan de "thought of the day" (`app/api/bot/uitdaging/route.ts`) viel net buiten de oude, eng geformuleerde trigger, terwijl de oude TECHNICAL_HANDOVER-beschrijving van die route toen al feitelijk onjuist was. Reden om het niet aan het maandagvangnet over te laten: "volgende week in een PR" is structureel slechter dan "in dezelfde commit". Bij twijfel of een wijziging "merkbaar" genoeg is: kort even bij Arno checken, niet zelf wegredeneren.
...
| `app/api/bot/uitdaging/route.ts` | `claude-fable-5` | Grammaticale kwaliteit en voortgangsherkenning vereisen Fable. max_tokens 600 (thinking telt mee). Prompt uitgebreid met taalcontrole en progressie-instructie. Had wel een refusal-check maar geen check op een leeg-maar-niet-refusal antwoord; nu alsnog retry-bij-leeg-antwoord en een expliciete foutrespons (niet opgeslagen) bij aanhoudend leeg antwoord. **Getest tegen `claude-opus-5` (2026-07-26, `scripts/test-opus5-vs-fable5.mjs`, blinde vergelijking op echte data):** Fable 5 gehandhaafd. Fable 5's vraag was korter en directer (1 zin vs. 2), sloot beter aan bij Arno's stem. Verworpen: overstap naar Opus 5. **Toon en drempel herzien (2026-08-29):** dit is de "thought of the day" op de coachingpagina. Arno's klacht: te kritisch/confronterend, voelde als een verhoor, en verscheen al bij 1 gesprek over een onderwerp dat de gebruiker niet als "besproken" herkende. Nu: (1) toon-instructie omgezet naar inspirerend "mag schuren maar niet irriteren, geen stroop om de mond, geen woorden als blinde vlek/zwakte", (2) format niet meer verplicht "1 inzicht + 1 vraag", mag ook een losse gedachte zonder vraagteken zijn (past bij de naam "thought", niet "question"), (3) weekend-variant niet langer "filosofisch confronterend" maar perspectief-gevend met een eerlijke ondertoon, (4) **drempel:** weekend altijd generiek (alleen profiel-rol als context, geen gesprekken/coachingdoc/analyse), doordeweeks personaliseren op patronen pas vanaf 3 gesprekken, daaronder ook generiek. Bijvangst: `deleted_at is null`-filter ontbrak op de sessie-query (telde verwijderde gesprekken mee), en de twee verplichte markdown/streepje-zinnen ontbraken in de systeemprompt, beide toegevoegd.
