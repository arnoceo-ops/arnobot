...
- **Env vars:** `NEXT_PUBLIC_POSTHOG_KEY` + `NEXT_PUBLIC_POSTHOG_HOST` (EU-regio: `https://eu.i.posthog.com`, Frankfurt). Ontbreken de env vars, dan initialiseert de tracker niet (geen crash, gewoon stil uit) — pas actief zodra Arno een PostHog-project heeft aangemaakt en de keys heeft toegevoegd aan `.env.local` en Vercel.
