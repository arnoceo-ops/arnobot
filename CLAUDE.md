...
**Push-getriggerd naast de wekelijkse planning (toegevoegd 2026-08-29):** dezelfde routine draait nu ook bij elke `push` naar de GitHub-repo (`arnoceo-ops/arnobot`), via een webhook-trigger (`d503d0e8-77fd-4e84-8e8a-074a791a1fbe`, hook_type `app`, source `github`) die dezelfde routine (`trig_01JzhR9LK17sT5g1MfQVSa9N`) afvuurt. Reden: het wekelijkse venster kon tot 7 dagen drift laten staan; nu wordt drift gevangen binnen minuten na de commit die 'm veroorzaakt, terwijl de PR-review als vangnet blijft (geen blinde auto-commit). De maandagplanning blijft als extra net staan. **Bekende beperkingen van de push-trigger:** (1) de webhook-API accepteert geen branch-filter, dus de routine vuurt op een push naar élke branch, niet alleen master, maar de routine checkt intern altijd master's laatste 7 dagen en eindigt stil bij een schone diff, dus dat is hooguit een paar verspilde runs; (2) blijft echte drift meerdere pushes lang onopgelost (Arno merget de PR niet meteen), dan kan de routine meerdere PR's voor dezelfde drift openen. Als dat in de praktijk hindert: de routine-prompt uitbreiden met een `gh pr list`-check die een bestaande open documentatie-versheids-PR bijwerkt in plaats van een nieuwe te openen (nu bewust niet gedaan, om de werkende routine-prompt niet te riskeren voor een probleem dat zich nog niet heeft voorgedaan).
