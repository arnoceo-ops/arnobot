export const PERSONA_BESCHRIJVINGEN: Record<string, Record<string, string>> = {
  verkoper: {
...
export const WEERSTAND_OPENING_TOON: Record<string, string> = {
  licht: 'Open vriendelijk en toegankelijk. Warme, uitnodigende toon, alsof je oprecht open staat voor het gesprek.',
  stevig: 'Open zakelijk en neutraal. Geen vijandigheid, maar ook geen warmte, je wacht af wat er komt.',
  zwaar: 'Open direct met wat wrijving of ongeduld al voelbaar in de eerste zin, alsof je al sceptisch bent nog voor het gesprek goed en wel begonnen is.',
}

export function getPersonaBeschrijving(rolCategorie: string, persona: string, context?: string | null): string {
  return persona === 'anders' && context
