...
export async function POST() {
  ...
  if (recent.length >= 4) {
    if (aantalNee >= 3) {
      actieOpvolgingContext = `\n\nACTIEOPVOLGING: De gebruiker geeft bij het begin van sessies aan of de vorige actie is gedaan. Recente antwoorden (meest recent eerst): ${recentLabels}. Dit is een duidelijk patroon: acties worden structureel niet opgepakt. Transparantie is een kernwaarde van Arno. Benoem dit direct in de actie_diagnose zonder omwegen. Geen aannames over de reden, maar het patroon zelf is een feit. Maak geen oordeel over de persoon, wel over het gedrag. De context kan anders zijn dan zichtbaar is, dus stel ook een directe vraag: "Wat houdt je structureel tegen?"`
    } else if (aantalDeels >= 3) {
      actieOpvolgingContext = `\n\nACTIEOPVOLGING: De gebruiker geeft bij het begin van sessies aan of de vorige actie is gedaan. Recente antwoorden (meest recent eerst): ${recentLabels}. Patroon: acties worden structureel ingepland maar niet afgerond. Benoem dit direct in de actie_diagnose. Geen aannames over de reden, maar het patroon is een feit. Vraag direct: zijn de acties te groot, of is er iets anders aan de hand?`
    } else if (aantalJa >= 4) {
