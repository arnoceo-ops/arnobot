...
export async function GET() {
  ...
  if (memberIds.length === 0) return NextResponse.json({ onvoldoende: true })
  ...
  if (totaalMetThema < DREMPEL_SESSIES) {
    return NextResponse.json({ onvoldoende: true, huidigAantal: totaalMetThema, benodigd: DREMPEL_SESSIES })
  }
