...
export default async function AdminStatusPage() {
  ...
  return (
    <main style={{ background: '#111827', minHeight: '100vh', color: '#f1f5f9', fontFamily: 'sans-serif' }}>
      <AdminMobileNav active="/bot/admin/status" />
      ...
      <div className="admin-content" style={{ maxWidth: '800px', margin: '0 auto', padding: '48px 40px' }}>
        ...
        {/* arno.bot metrics */}
        <p style={{ fontFamily: 'Space Mono, monospace', fontSize: 12, letterSpacing: 3, color: '#6b7280', marginBottom: 12 }}>ARNO.BOT</p>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 40 }}>
          <div style={{ background: '#1f2937', borderRadius: 4, padding: '20px', display: 'flex', gap: 20, alignItems: 'center' }}>
            <div style={{ flex: 1 }}>
              <p style={{ fontFamily: 'Space Mono, monospace', fontSize: 12, letterSpacing: 3, color: '#f59e0b', marginBottom: 16 }}>PERFORMANCE</p>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                <div>
                  <p style={{ fontFamily: 'Space Mono, monospace', fontSize: 10, letterSpacing: 2, color: '#6b7280', marginBottom: 2 }}>GEMIDDELD</p>
                  <p style={{ fontFamily: "'Bebas Neue', Impact, sans-serif", fontSize: 24, color: '#f1f5f9', lineHeight: 1 }}>{avgMs ? `${avgMs}ms` : '—'}</p>
                </div>
                <div>
                  <p style={{ fontFamily: 'Space Mono, monospace', fontSize: 10, letterSpacing: 2, color: '#6b7280', marginBottom: 2 }}>P95</p>
          ...
          <div style={{ background: '#1f2937', borderRadius: 4, padding: '20px', display: 'flex', gap: 20, alignItems: 'center' }}>
            <div style={{ flex: 1 }}>
              <p style={{ fontFamily: 'Space Mono, monospace', fontSize: 12, letterSpacing: 3, color: '#f59e0b', marginBottom: 16 }}>BESCHIKBAARHEID</p>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                <div>
                  <p style={{ fontFamily: 'Space Mono, monospace', fontSize: 10, letterSpacing: 2, color: '#6b7280', marginBottom: 2 }}>VANDAAG</p>
                  <p style={{ fontFamily: "'Bebas Neue', Impact, sans-serif", fontSize: 24, color: '#f1f5f9', lineHeight: 1 }}>{availDay !== null ? `${availDay}%` : '—'}</p>
                </div>
                <div>
                  <p style={{ fontFamily: 'Space Mono, monospace', fontSize: 10, letterSpacing: 2, color: '#6b7280', marginBottom: 2 }}>DEZE WEEK</p>
                  <p style={{ fontFamily: "'Bebas Neue', Impact, sans-serif", fontSize: 24, color: '#f1f5f9', lineHeight: 1 }}>{availWeek !== null ? `${availWeek}%` : '—'}</p>
                </div>
                <div>
                  <p style={{ fontFamily: 'Space Mono, monospace', fontSize: 10, letterSpacing: 2, color: '#6b7280', marginBottom: 2 }}>DOWN</p>
          ...
          {services.map(s => (
            <div key={s.name} style={{ background: '#1f2937', borderRadius: 4, padding: '16px 20px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: s.components.length > 0 ? 12 : 0 }}>
                <span style={{ width: 8, height: 8, borderRadius: '50%', background: indicatorColor(s.indicator), flexShrink: 0 }} />
                <span style={{ fontFamily: 'Space Mono, monospace', fontSize: 13, color: '#f1f5f9', fontWeight: 700 }}>{s.name}</span>
                <span style={{ fontFamily: 'Space Mono, monospace', fontSize: 12, color: '#6b7280' }}>{s.description}</span>
                <a href={s.link} target="_blank" rel="noopener noreferrer"
                  style={{ marginLeft: 'auto', fontFamily: 'Space Mono, monospace', fontSize: 11, color: '#6b7280', textDecoration: 'none', letterSpacing: 2, flexShrink: 0 }}>
                  STATUS →
                </a>
              </div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 4, paddingLeft: 18 }}>
                  {s.components.map(c => (
                    <div key={c.name} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      ...
                      <span style={{ fontFamily: 'Space Mono, monospace', fontSize: 11, color: '#9ca3af' }}>{c.name}</span>
                      {c.status !== 'operational' && (
                        <span style={{ fontFamily: 'Space Mono, monospace', fontSize: 11, color: componentColor(c.status) }}>{componentLabel(c.status)}</span>
                      )}
                    </div>
                  ))}
                </div>
              ...
              {s.lastIncident && (
                <p style={{ fontFamily: 'Space Mono, monospace', fontSize: 11, color: '#f59e0b', marginTop: 10, paddingLeft: 18 }}>
